...
CAUTION: We don't gain performance due to use an interface or the other one.

TIP: We gain performance because <<Externalizable>> interface forces us to implement ourselves the guessing code, so {Java} doesn't have to do that. 

CAUTION: As we have seen at <<Utilizando el interfaz Externalizable (correctamente)>>, si no tenemos cuidado, conseguiremos una mejora poco significativa a costa de complicar nuestro código fuente.
...
As we have seen at <<Implementing Externalizable (wrong way)>>, if we are not careful, we might get a almost no performance gain and we will be writing more code for nothing.
