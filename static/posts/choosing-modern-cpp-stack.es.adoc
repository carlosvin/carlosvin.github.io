...
{gradle}::
  La construcción de proyectos C++ está todavía en desarrollo, los modelos y APIs están cambiando. No es muy rápido. Puedes ver un ejemplo en este artículo link:/langs/{lang}/posts/gradle-cpp[Construir un proyecto Cpp con Gradle].
{make_link}::
